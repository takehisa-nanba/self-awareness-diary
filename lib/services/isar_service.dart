...
class IsarService {
  ...
  Isar get isar {
    if (_isar == null) {
      throw Exception("IsarService: Isar has not been initialized. Call init() first.");
    }
    return _isar!;
  }
  ...
  Future<void> init() async {
    ...
    _isar = await Isar.open(
      [DiaryRecordSchema, LocationSettingSchema, AppSettingsSchema],
      directory: dir.path,
    );
  ...
  Future<String?> getSetting(String key) async {
    final setting = await isar.collection<AppSettings>().where().keyEqualTo(key).findFirst();
  ...
  Future<List<DiaryRecord>> getAllRecords() async {
    try {
      return await isar.collection<DiaryRecord>().where().sortByRecordDateDesc().findAll();
    } catch (e) {
  ...
  Future<List<DiaryRecord>> getRecordsByDate(DateTime date) async {
    ...
    return await isar.collection<DiaryRecord>()
        .filter()
